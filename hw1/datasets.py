...
def torch_temporary_seed(seed: int):
    ...
    # ====== YOUR CODE: ======
    org_seed = torch.random.get_rng_state()
    # ========================
    try:
        yield torch.random.manual_seed(seed)
    finally:
        torch.random.set_rng_state(org_seed)
...
class ImageStreamDataset(IterableDataset):
    ...
    def __next__(self):
        self.i += 1
        with torch_temporary_seed(self.i):
            return random_labelled_image(self.image_dim, self.num_classes)
